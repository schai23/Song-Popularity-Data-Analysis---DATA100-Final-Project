...
We start the data processing flow by reading the CSV files and converting them into a consistent format. The function reads and ensures all columns are then combnines into a single combined_data table. In this table, there are numeric columns such as "Duration (ms)" and "Popularity" which are converted from character to numeric. The "Release Date" is also formatted as a date from a string. The columns such as Popularity, Artist Names, and Track Name are kept in the dataset for further analysis because they are important.
...
Attached are some extra graphs that show the relation between Artist Count and Popularity to allow us to specify a relation between the two.

### Exploratory Linear Model 2 - Linear Model
